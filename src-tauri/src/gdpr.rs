...
impl GdprManager {
    ...
    pub async fn export_student_data_csv(&self, db: &Database, student_id: i64) -> Result<String> {
    ...
    pub async fn generate_compliance_report(&self, db: &Database) -> Result<GdprComplianceReport> {
        let students = db.get_students().await?;
        ...
        if let Some(oldest) = oldest_observation {
            ...
            if age_days > policy.anonymization_after_days as i64 {
                recommendations.push(
                    "Consider anonymizing data older than 3 years to maintain GDPR compliance"
                        .to_string(),
                );
            }
        }
        ...
        if observations_count > 0 {
            recommendations.push(
                "Regular data retention review scheduled - verify observation retention periods"
                    .to_string(),
            );
        }
...
mod tests {
    ...
    async fn test_soft_delete_student() {
        ...
        assert!(result
            ...
            .as_ref()
            .unwrap()
            .contains("1 observations retained"));
    ...
    async fn test_validate_data_subject_request() {
        ...
        let valid_export = gdpr
            .validate_data_subject_request(123, "export")
            .await
            .unwrap();
        assert!(valid_export);

        let valid_delete = gdpr
            .validate_data_subject_request(123, "delete")
            .await
            .unwrap();
        assert!(valid_delete);

        let valid_rectify = gdpr
            .validate_data_subject_request(123, "rectify")
            .await
            .unwrap();
    ...
    async fn test_export_full_database() {
        ...
        assert_eq!(
            export_active["data"]["students"].as_array().unwrap().len(),
            0
        );
    ...
    async fn test_calculate_data_processing_lawfulness() {
        ...
        let education_basis = gdpr
            .calculate_data_processing_lawfulness("education")
            .await
            .unwrap();
        assert_eq!(
            education_basis["legal_basis"],
            "Article 6(1)(e) - Public task"
        );
        assert_eq!(education_basis["documentation_required"], true);

        // Test student welfare
        let welfare_basis = gdpr
            .calculate_data_processing_lawfulness("student_welfare")
            .await
            .unwrap();
        assert_eq!(
            welfare_basis["legal_basis"],
            "Article 6(1)(c) - Legal obligation"
        );

        // Test legitimate interest
        let legitimate_basis = gdpr
            .calculate_data_processing_lawfulness("legitimate_interest")
            .await
            .unwrap();
        assert_eq!(
            legitimate_basis["legal_basis"],
            "Article 6(1)(f) - Legitimate interests"
        );
        assert_eq!(legitimate_basis["balancing_test_required"], true);

        // Test unknown purpose
        let unknown_basis = gdpr
            .calculate_data_processing_lawfulness("unknown")
            .await
            .unwrap();
        assert_eq!(unknown_basis["legal_basis"], "Not determined");
        assert!(unknown_basis["action_required"]
            .as_str()
            .unwrap()
            .contains("assessment"));
    }
