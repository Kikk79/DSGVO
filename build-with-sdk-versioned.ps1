...
# Run the versioned build
npm run tauri:build:versioned
if ($LASTEXITCODE -eq 0) {
    Write-Host "Tauri build completed successfully!" -ForegroundColor Green
}
else {
    Write-Host "Tauri build failed with exit code $LASTEXITCODE" -ForegroundColor Red
}
Write-Host "Copying installer files to installer directory..." -ForegroundColor Green
cp .\src-tauri\target\release\bundle\nsis\* .\installer\
cp .\src-tauri\target\release\bundle\msi\* .\installer\ 
Write-Host "Installer files copied successfully!" -ForegroundColor Green